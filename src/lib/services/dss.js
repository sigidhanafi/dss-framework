...
export const calculateDss = async (dssId, method) => {
  ...
  if (method == DssMethodType.WP) {
    methodResult = Object.entries(
      calculateWP(weightMap, alternatives, criterias)
    );
  } else if (method == DssMethodType.SAW) {
    const normalizedAlternatives = normalizeSawAlternatives(
      criterias,
      alternatives
    );
    methodResult = Object.entries(
      calculateSAW(weightMap, normalizedAlternatives, criterias)
    );
  } else if (method == DssMethodType.TOPSIS) {
    methodResult = Object.entries(calculateTopsis(alternatives, criterias)).map(
      (item) => item[1]
    );
  }
...
function calculateAggregatedValues(alternatives, criterias) {
  let criteriaMap = Object.fromEntries(
    criterias.map((c) => [c.criteriaId, { ...c, children: [] }])
  );
...
function calculateTopsis(alternatives, criterias) {
  ...
  const criteriaIds = criterias
    .filter((c) => c.parentCriteriaId === null)
    .map((c) => c.criteriaId);
  ...
  criteriaIds.forEach((cId) => {
    sumSquares[cId] = Math.sqrt(
      alternatives.reduce(
        (sum, alt) => sum + Math.pow(alt.values[cId] || 0, 2),
        0
      )
    );
  });
  ...
  let normalizedMatrix = alternatives.map((alt) => ({
    ...
    values: Object.fromEntries(
      criteriaIds.map((cId) => [
        cId,
        (alt.values[cId] || 0) / (sumSquares[cId] || 1),
      ])
    ),
  }));
  ...
  criteriaIds.forEach((cId) => {
    let isBenefit =
      criterias.find((c) => c.criteriaId === cId)?.type === 'BENEFIT';
  ...
  let scores = weightedMatrix.map((alt) => {
    let dPlus = Math.sqrt(
      criteriaIds.reduce(
        (sum, cId) => sum + Math.pow(alt.values[cId] - idealBest[cId], 2),
        0
      )
    );
    let dMinus = Math.sqrt(
      criteriaIds.reduce(
        (sum, cId) => sum + Math.pow(alt.values[cId] - idealWorst[cId], 2),
        0
      )
    );
    return {
